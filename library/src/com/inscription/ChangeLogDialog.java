...
public class ChangeLogDialog {
	...
	private String parseDate(final String dateString) {
		...
		try {
			...
			return DateFormat.getDateFormat(getContext()).format(
					parsedDate);
	...
	private void parseReleaseTag(
			final StringBuilder changelogBuilder,
			final XmlPullParser resourceParser)
			throws XmlPullParserException, IOException {
		changelogBuilder
				.append("<h1>Release: ")
				.append(resourceParser.getAttributeValue(null,
						"version")).append("</h1>");
		...
		if (resourceParser.getAttributeValue(null, "date") != null) {
			changelogBuilder
					...
					.append(parseDate(resourceParser
	...
	protected void show(final int version) {
		...
		final String htmlChangelog = getHTMLChangelog(
				R.xml.changelog, resources, version);
		...
		webView.loadDataWithBaseURL(null, htmlChangelog, "text/html",
				"utf-8", null);
		final AlertDialog.Builder builder = new AlertDialog.Builder(
				mContext)
				.setTitle(title)
				.setView(webView)
				.setPositiveButton(closeString,
